...
<head>
    ...
    <script type="text/javascript"
            src="${pageContext.request.contextPath}/static/js/admin/main.js"></script>
...
// 		alert(uid);
            $.ajax({
                type : "get",
                url : "/" + window.document.location.pathname.split("/")[1] + "/userManage/toEditPage.action",
                data : {"uid":uid},
...
// 		alert("uid:"+uid);
            ...
            if (confirm('确认要删除该用户吗?')) {
                ...
                $.ajax({
                    type : "get",
                    url : "/" + window.document.location.pathname.split("/")[1] + "/userManage/ajaxConfirmDeleteUser.action",
                    data : {"uid":uid},
                    contentType:"application/json;charset=utf-8",/* 发送数据给服务器时所用的内容类型	*/
                    dataType : "json",//返回时的数据类型json
                    success : function(data) {
// 					alert("成功");
                        if(data.name=="yes"){//可以删除
                            alert("删除用户成功！");
                            //跳转到删除用户action
                            window.location.href= "/" + window.document.location.pathname.split("/")[1] + "/userManage/deleteUser.action?uid="+uid+"&currentPage2="+currentPage2;
                        }
                        else{
                            ...
                            >
                                ...
                                <div class="modal-body"><!-- 在这里添加一些文本 -->
                                    ...
                                    <div class="form-group">
                                        <label for="email">邮箱</label>
                                        <input type="email" class="form-control" placeholder="请输入邮箱"
                                               name="email" id="update_email" required="required">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        <label for="phone">手机</label>
                                        <input type="text" class="form-control" placeholder="请输入手机号"
                                               name="phone" id="update_phone" required="required">
                                    </div>
